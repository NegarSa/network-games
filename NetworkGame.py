...
class NetworkGame():
    '''
        Implemets the base case of a network game,
    '''
    def __init__(self, network, pi, f, init_state = None):
        ...
        '''
        self.net =
    def __init__(self, net, pi, init_state = None):
        self.net = copy.deepcopy(net)
        self.init_pi = copy.deepcopy(pi)
        self.pi = copy.deepcopy(pi)
        self.n = nx.number_of_nodes(self.net)
        if init_state is None:
            #self.x = np.random.choice([0, 1], size=(self.n,), p=[1./3, 2./3])
    ...
    def nA(self, i):
        count = 0
        for v in nx.neighbors(self.net, i):
            if self.x[v] == 0:
                count += 1
        return count
